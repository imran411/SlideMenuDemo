...
public class SlideMenuAnimationContainer extends ViewGroup {
    ...
    public void onFinishInflate() {
        ...
        if (slideMenuLinerLayout == null) {
            throw new NullPointerException(
                    "Activity must have a LinearLayout with id:slideMenuLinearLayout in which the menu will be embedded");
        }

        contentLinearLayout = findViewById(R.id.contentLinearLayout);
        if (contentLinearLayout == null) {
            throw new NullPointerException(
                    "Activity must have a LinearLayout with id:contentLinearLayout in which content will be embedded");
        }
    ...
    public void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        slideMenuLinerLayout.layout(l, 0, l + slideMenuWidth, 0 + slideMenuLinerLayout.getMeasuredHeight());
        if (isOpened) {
            contentLinearLayout.layout(l + slideMenuWidth, 0, r + slideMenuWidth, b);
        }
        else {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        int x = (int) ev.getX();
        int y = (int) ev.getY();
        if (contentLinearLayout.getLeft() < x && contentLinearLayout.getRight() > x && contentLinearLayout.getTop() < y
                && contentLinearLayout.getBottom() > y) {
